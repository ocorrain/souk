...
(defun make-druid (on-complete &rest entries)
  (make-instance 'druid :data entries :on-complete on-complete))
...
(defmethod druid-render-status ((druid druid) (type (eql :simple)))
  ...
  (with-html
    (:div :class "status"
      (esc (format nil "Step ~D" (druid-current-step druid))))))
...
(defmethod druid-render-status ((druid druid) (type (eql :simple-with-total)))
  ...
  (with-html
    (:div :class "status"
	  (esc (format nil "Step ~D/~D" (druid-current-step druid)
		   (druid-total-steps druid))))))

(defmethod druid-current-step ((druid druid))
  (1+ (druid-data-cursor druid)))
...
(defmethod render-widget-body ((druid-entry druid-entry) &rest args)
  (declare (ignore args))
  (funcall (data-edit-function druid-entry) (druid-entry-data druid-entry))
